...
if st.button('Predict Price'):
    ...
    query = np.array([company,typename,ram,weight,ips,touchscreen,ppi,cpu,hdd,ssd,gpu,os])

    query = query.reshape(1,12)
    ans = pipe.predict(query)[0]
    st.title("The predicted price of this configuration is " + str(int(np.exp(ans))))
